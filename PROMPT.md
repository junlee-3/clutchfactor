...
- [x] **M5 — Reference corpus & D6.** Corpus screen + ingestion; occupancy grids per map/side/phase; D6 with honesty rules + minimum-corpus gate; heatmap rendering (invoke `dataviz` first). **DoD: with ~8 pro demos on one map, positioning comparison produces sane, replay-backed output on the owner's demo.** (Implementation + E2E verified 2026-08-20 with a documented dev-only gate override against 1 pro mirage demo — outputs matched an independent SQL recomputation; the 8-demo corpus ask to the owner is outstanding, shipped gate stays 8.)
- [x] **M6 — Trends, polish, ship.** Trends screen; Settings (thresholds, identity); empty/error states everywhere (corrupt demo, wrong game, mid-parse crash recovery); app icon; tagged release with Windows installer + mac app; README with screenshots. **DoD: owner installs the Windows build from CI artifacts on their gaming PC and analyzes a fresh match unassisted.**
