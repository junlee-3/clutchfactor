...
- Library map-preview icons (`previews/*.png`) are the CS2 in-game map-selection
  / match-history badges, extracted from the official game depot and vendored
  from [MurkyYT/cs2-map-icons](https://github.com/MurkyYT/cs2-map-icons)
  (`images/<map>.png`, 512×512). They replace the flat radar tiles on Library
  rows only — the replay viewer still uses the awpy radar PNGs.
...
- To update radars for a new CS2 patch: check `awpy/data/__init__.py` for the
  ...
  pull the matching `images/<map>.png` files from `MurkyYT/cs2-map-icons` into
  `previews/`.
