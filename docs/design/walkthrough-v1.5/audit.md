...
## In-app verification (controller, 2026-08-27)

Run from this branch with `VITE_FAIL_IPC=<command> pnpm tauri dev`, driven through the accessibility API under the owner-activity guard (no action unless the Mac had been idle ≥ 5 minutes).

| Capture | State | What it shows |
|---|---|---|
| `audit/library-error.png` | Library · error | "Couldn't load your matches" · reason `forced failure: list_matches` · Retry — header and Import action still in place |
| `audit/report-loading.png` | Report · loading | header + card skeletons at final size |
| `audit/trends-loading.png` | Trends · loading | ribbon + card skeletons at final size |
| `audit/replay-loading-first-load.png` | Replay · loading (first round) | header, strip and round-area skeleton at final size |
| `audit/replay-loading-after-switch.png` | Replay · loading (after switching to round 2) | header and strip stay mounted; only the round area shows the skeleton |

**Finding from this pass:** with TanStack Query's default `retry: 3` (backoff 1 s / 2 s / 4 s), a failed local command showed the skeleton for about seven seconds before the error state — the four captures above were meant to be error states and caught the skeleton instead. Local commands fail deterministically, so the `QueryClient` now sets `retry: false` (commit "no query retries for local commands"); the Retry action is the retry. The error states for Report, Trends, Replay, Corpus, Settings and Watches are verified by code review (same branch order and component as the Library one captured above) and by the `call()` forced-failure unit test; their in-app captures were not re-taken because the owner was using the Mac — re-run `VITE_FAIL_IPC=get_match_report|get_trends|get_round_ticks|corpus_status|get_app_settings|get_detector_catalog pnpm tauri dev` from a quiet desktop to see each one.
